...
module Processor (
	...
	input  logic [7:0]  Din,      //change to [7:0]  
	input  logic [2:0]  F,       // Function select 
	input  logic [1:0]  R,       // Routing select

	output logic [3:0]  LED,     // DEBUG   
	output logic [7:0]  Aval,    // DEBUG  //change to 7:0
	output logic [7:0]  Bval,    // DEBUG //change to 7:0
	...
	logic       opA;
	logic       opB;
	logic [7:0] A;     //change to 7:0
	logic [7:0] B;     //change to 7:0
	logic [7:0] Din_S;    //change to 7:0   
...
//When you extend to 8-bits, you will need more HEX drivers to view upper nibble of registers, for now set to 0
	HexDriver HexA (
		...
		.in         ({A[7:4], A[3:0], 4'b0000, 4'b0000}),   //change i think 7:4?
		.hex_seg    (hex_seg),
		.hex_grid   (hex_grid)
	);
	...
	HexDriver HexB (
		...
		.in         ({B[7:4], B[3:0], 4'b0000, 4'b0000}),   //change i think 7:4?
		.hex_seg    (hex_seg2),
		.hex_grid   (hex_grid2)
	);
...
//Note: S stands for SYNCHRONIZED, H stands for active HIGH, in addition in synthesis they are also debounced
	...
	sync_debounce Din_sync [7:0] (    //change 
		.Clk  (Clk), 
